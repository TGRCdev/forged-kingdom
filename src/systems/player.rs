...
pub fn setup_player(
    ...
    for ent in query.iter() {
        if let Some(player) = *main_player {
            panic!(
                "Tried to initialize a second player {} (Existing player {})",
                ent, player
            );
        }
        ...
        let player_camera = commands
            .spawn((PlayerCamera::default(), Transform::from_xyz(0.0, 0.5, 0.0)))
            .id();
        ...
        commands
            ...
            .insert(MeshMaterial3d(
                materials.add(Color::linear_rgb(0.0, 0.0, 1.0)),
            ))
