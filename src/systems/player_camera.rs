...
pub fn rotate_player_camera_pivot(
    mut cam_query: Query<(&mut Transform, &ActionState<PlayerCameraPivotAction>), With<PlayerCameraPivot>>,
) {
    for (mut cam_xform, action_state) in cam_query.iter_mut()
    {
        match action_state.dual_axis_data(&PlayerCameraPivotAction::Rotate).map(|data| data.update_pair)
        {
            None | Some(Vec2::ZERO) => (),
...
pub fn player_camera_handle_zoom_action(
    ...
    {
        if let Some(zoom_val) = action_state.axis_data(&PlayerCameraAction::Zoom)
            .and_then(|data| if data.update_value.is_normal() { Some(data.update_value) } else { None })
        {
...
/// zoom level.
pub fn player_camera_lerp_zoom(
    mut cam_query: Query<(&PlayerCamera, &mut Transform)>
) {
    cam_query.iter_mut().for_each(|(cam, mut xform)| {
        xform.translation.z = xform.translation.z
